...
% Data
    ...
    DataMat = single(cell2mat(Data(2:rowsData,4:colsData)));
...
for n=1:rowsDataMat
        ...
        for c = 1:48
            ...
            for m = 1:30
                ...
                DataMatmm30(n,mm)= single(DataMatmm(n,mm)/30); % kWh Useage Per Minute 29.995 used to give better fit.
                % Use a Normally Distributed Random Number to show Change in
                % Usegae Minute by Minute
                livedatause(n,mm) = single(n2(n,m)/30);
                livedatadem(n,mm) = 2*n2(n,m);
            end
        end
    
%         datafile=v2struct(livedatause, DataMatmm30, DataMat,sdate);
end
